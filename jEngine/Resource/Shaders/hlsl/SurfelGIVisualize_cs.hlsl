...
{
    return max((uint)VisualizeCommon.CascadePartitionCapacity, 1u);
}
...
{
    int dimX = max((int)round(SurfelGIGetPackedFloat(VisualizeCommon.CascadeClipmapGridDimXPacked, cascadeIndex)), 1);
    int dimY = max((int)round(SurfelGIGetPackedFloat(VisualizeCommon.CascadeClipmapGridDimYPacked, cascadeIndex)), 1);
    int dimZ = max((int)round(SurfelGIGetPackedFloat(VisualizeCommon.CascadeClipmapGridDimZPacked, cascadeIndex)), 1);
...
uint GetClipmapLocalLinearIndex(int3 cellCoord, uint maxSurfels, uint desiredSlotsPerCell, uint cascadeIndex)
{
    ...
    const float cellSize = max(VisualizeCommon.GridCellSize, 0.1) * SurfelGIGetCascadeScale(VisualizeCommon.CascadeCellScalePacked, cascadeIndex);
...
bool TryWorldCellToLinear(int3 worldCell, uint cascadeIndex, out uint outCellLinear)
{
    return SurfelGITryWorldCellToLinear(
        worldCell,
        cascadeIndex,
        VisualizeCommon.CascadeClipmapGridDimXPacked,
        VisualizeCommon.CascadeClipmapGridDimYPacked,
        VisualizeCommon.CascadeClipmapGridDimZPacked,
        VisualizeCommon.CascadeOriginCellXPacked,
        VisualizeCommon.CascadeOriginCellYPacked,
        VisualizeCommon.CascadeOriginCellZPacked,
        VisualizeCommon.CascadeRingOffsetXPacked,
        VisualizeCommon.CascadeRingOffsetYPacked,
        VisualizeCommon.CascadeRingOffsetZPacked,
        VisualizeCommon.CascadeCellBasePacked,
        outCellLinear);
}
...
bool TryGetCellBaseIndex(int3 cellCoord, uint maxSurfels, uint cascadeIndex, out uint outCellBaseIndex)
{
    return SurfelGITryGetCellBaseIndex(
        cellCoord,
        maxSurfels,
        GetPageSize(maxSurfels),
        (uint)VisualizeCommon.SurfelPageTableCapacity,
        cascadeIndex,
        VisualizeCommon.CascadeClipmapGridDimXPacked,
        VisualizeCommon.CascadeClipmapGridDimYPacked,
        VisualizeCommon.CascadeClipmapGridDimZPacked,
...
{
    const uint desiredSlotsPerCell = SurfelGIGetDesiredSlotsPerCell(VisualizeCommon.SurfelsPerCellPacked, cascadeIndex);
    const uint cascadeCapacity = GetCascadePartitionCapacity();
...
void main(uint3 GlobalInvocationID : SV_DispatchThreadID)
{
    ...
    const uint debugCascadeIndex = SurfelGIGetCascadeIndexByDistance(VisualizeCommon.CascadeStartDistancePacked, cameraDistance);
    const float debugCellSize = cascade0CellSize * SurfelGIGetCascadeScale(VisualizeCommon.CascadeCellScalePacked, debugCascadeIndex);
    ...
    {
        const float cascadeScale = SurfelGIGetCascadeScale(VisualizeCommon.CascadeCellScalePacked, cascadeIndex);
                    ...
                    [loop] for (uint slot = 0u; slot < desiredSlotsPerCell; ++slot)
                    {
                        ...
                        if (d2 <= r2 && d2 < bestDist2)
                        {
                            ...
                            if (VisualizeCommon.ShowStateDebug != 0 && VisualizeCommon.ShowCellDebug == 0 && VisualizeCommon.ShowIrradianceDebug == 0)
                            {
                                ...
                                if (state == SURFEL_GI_SURFEL_STATE_NEW)
                                    stateTint = float3(1.0, 0.2, 0.2);
                                else if (state == SURFEL_GI_SURFEL_STATE_STABLE_KEEP)
                                    stateTint = float3(0.2, 1.0, 0.2);
                                else if (state == SURFEL_GI_SURFEL_STATE_MERGED_MOVED)
                                    stateTint = float3(1.0, 0.95, 0.2);
                                else if (state == SURFEL_GI_SURFEL_STATE_REPLACED_FAR)
                                    stateTint = float3(0.2, 1.0, 1.0);
                                else if (state == SURFEL_GI_SURFEL_STATE_STOLEN_FAR)
                                    stateTint = float3(1.0, 0.2, 1.0);
                                else if (state == SURFEL_GI_SURFEL_STATE_DORMANT)
    ...
    {
        const float debugCascadeScale = SurfelGIGetCascadeScale(VisualizeCommon.CascadeCellScalePacked, debugCascadeIndex);
        ...
        const uint debugDesiredSlots = SurfelGIGetDesiredSlotsPerCell(VisualizeCommon.SurfelsPerCellPacked, debugCascadeIndex);
        ...
        [loop] for (uint slot = 0u; slot < debugDesiredSlots && hasDebugCellPage; ++slot)
        {
            ...
            const float3 surfelNormal = normalize(s.Normal);
    ...
    if (VisualizeCommon.ShowCellGrid != 0 && VisualizeCommon.ShowIrradianceDebug == 0)
    {
        ...
        const float gridCellSize = cascade0CellSize * SurfelGIGetCascadeScale(VisualizeCommon.CascadeCellScalePacked, resolvedGridCascadeIndex);
