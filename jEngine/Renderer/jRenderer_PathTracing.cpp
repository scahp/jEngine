...
void jRenderer_PathTracing::PathTracing()
{
	...
	// Bindless
	jShaderBindingArray BindlessShaderBindingArray[5];
	...
	std::vector<std::shared_ptr<IUniformBufferBlock>> Temp;
	
	{
		...
		for (int32 i = 0; i < NumOfStaticRenderObjects; ++i)
		{
			...
			auto RenderObjectUniformParametersPtr = g_rhi->CreateUniformBufferBlock(jNameStatic("jMaterialUniformBuffer"), jLifeTimeType::OneFrame, RObj->MaterialPtr->MaterialDataPtr->GetDataSizeInBytes());
			RenderObjectUniformParametersPtr->UpdateBufferData(RObj->MaterialPtr->MaterialDataPtr->GetData(), RObj->MaterialPtr->MaterialDataPtr->GetDataSizeInBytes());
			MaterialBuffers.push_back(RenderObjectUniformParametersPtr.get());
			Temp.push_back(RenderObjectUniformParametersPtr);
		}
	...
	RaytracingPipelineData.MaxPayloadSize = 4 * sizeof(float) + sizeof(uint32);
	RaytracingPipelineData.MaxTraceRecursionDepth = 5;
	...
	{
		DEBUG_EVENT_WITH_COLOR(RenderFrameContextPtr, "CopyToFinalColorRT", Vector4(0.8f, 0.0f, 0.0f, 1.0f));
		SCOPE_CPU_PROFILE(CopyToFinalColorRT);
		SCOPE_GPU_PROFILE(RenderFrameContextPtr, CopyToFinalColorRT);
