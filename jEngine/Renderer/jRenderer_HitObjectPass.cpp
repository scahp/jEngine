...
void jRenderer::HitObjectPass()
{
	...
	if (IsUseDX12())
		{
			// Transition HitObject_RT to TRANSFER_SRC
			...
			// Setup copy region for single pixel
			...
			// Wait for GPU to finish (this is synchronous for now - can be optimized later)
			g_rhi->Flush();
			g_rhi->Finish();
			...
			if (MappedPtr)
			{
				// Decode RenderObjectID from RGBA
				...
				// Find RenderObject by ID and select it
	#ifdef ENABLE_EDITOR_FEATURES
				if (g_Editor)
				{
					if (renderObjectID != 0)
