...
std::map<size_t, std::list<jFrameBufferPool::jFrameBufferPoolResource> > jFrameBufferPool::FrameBufferResourceMap;
...
std::shared_ptr<jFrameBuffer> jFrameBufferPool::GetFrameBuffer(const jFrameBufferInfo& info)
{
	...
	auto renderTargetPtr = std::shared_ptr<jFrameBuffer>(g_rhi->CreateFrameBuffer(info));
	if (renderTargetPtr)
	{
