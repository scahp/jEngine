...
struct jTexture_DX12 : public jTexture
{
    ...
    jTexture_DX12(ETextureType InType, ETextureFormat InFormat, int32 InWidth, int32 InHeight, int32 InLayerCount, EMSAASamples InSampleCount, int32 InMipLevel, bool InSRGB, const jRTClearValue& InClearValue
        , const std::shared_ptr<jCreatedResource>& InImage, jDescriptor_DX12 InRTV = {}, jDescriptor_DX12 InDSV = {}, jDescriptor_DX12 InSRV = {}, jDescriptor_DX12 InUAV = {}/*, EResourceLayout InImageLayout = EResourceLayout::UNDEFINED*/)
        : jTexture(InType, InFormat, InWidth, InHeight, InLayerCount, InSampleCount, InMipLevel, InSRGB), Texture(InImage), RTV(InRTV), DSV(InDSV), SRV(InSRV), UAV(InUAV)
    {
    }
    ...
    virtual EResourceLayout GetLayout() const override { return Texture ? Texture->Layout : EResourceLayout::UNDEFINED; }
    FORCEINLINE void SetLayout(EResourceLayout InNewLayout) { if (Texture) Texture->SetLayout(InNewLayout); }
};
