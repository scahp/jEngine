...
GENERATE_CONVERSION_FUNCTION(GetDX12TextureFormat,
    ...
    CONVERSION_TYPE_ELEMENT(ETextureFormat::R10G10B10A2, DXGI_FORMAT_R10G10B10A2_UNORM),
    ...
    CONVERSION_TYPE_ELEMENT(ETextureFormat::R16UI, DXGI_FORMAT_R16_UINT),
...
FORCEINLINE auto GetDX12TextureComponentCount(ETextureFormat type)
{
    GENERATE_STATIC_CONVERSION_ARRAY(
        ...
        CONVERSION_TYPE_ELEMENT(ETextureFormat::R16UI, 1),
...
FORCEINLINE auto GetDX12TexturePixelSize(ETextureFormat type)
{
    GENERATE_STATIC_CONVERSION_ARRAY(
        ...
        CONVERSION_TYPE_ELEMENT(ETextureFormat::R8UI, 1),
        CONVERSION_TYPE_ELEMENT(ETextureFormat::R16UI, 2),
...
FORCEINLINE ETextureType GetDX12TextureDemension(D3D12_RESOURCE_DIMENSION InType, bool InIsArray)
{
    switch(InType)
    {
        case D3D12_RESOURCE_DIMENSION_TEXTURE2D:
        {
            return InIsArray ? ETextureType::TEXTURE_2D_ARRAY : ETextureType::TEXTURE_2D;
        }
        case D3D12_RESOURCE_DIMENSION_TEXTURE3D:
        {
            return InIsArray ? ETextureType::TEXTURE_3D_ARRAY : ETextureType::TEXTURE_3D;
        }
...
FORCEINLINE D3D12_RESOURCE_DIMENSION GetDX12TextureDemension(ETextureType InType)
{
    switch (InType)
    {
...
FORCEINLINE auto GetDX12PrimitiveTopologyTypeOnly(EPrimitiveType type)
{
    GENERATE_STATIC_CONVERSION_ARRAY(
...
GENERATE_CONVERSION_FUNCTION(GetDX12PolygonMode,
    CONVERSION_TYPE_ELEMENT(EPolygonMode::POINT, D3D12_FILL_MODE_WIREFRAME),    // 지원 안함
...
GENERATE_CONVERSION_FUNCTION(GetDX12CullMode,
    ...
    CONVERSION_TYPE_ELEMENT(ECullMode::FRONT_AND_BACK, D3D12_CULL_MODE_BACK))   // 지원 안함

FORCEINLINE auto GetDX12BlendFactor(EBlendFactor type)
{
    GENERATE_STATIC_CONVERSION_ARRAY(
        ...
        //CONVERSION_TYPE_ELEMENT(EBlendFactor::CONSTANT_ALPHA, ),                          DX12 지원안함.
        //CONVERSION_TYPE_ELEMENT(EBlendFactor::ONE_MINUS_CONSTANT_ALPHA, ),                DX12 지원안함.
...
FORCEINLINE uint8 GetDX12ColorMask(EColorMask type)
{
    uint8 result = 0;
    ...
    {
        if (!!(EColorMask::R & type)) result |= D3D12_COLOR_WRITE_ENABLE_RED;
        if (!!(EColorMask::G & type)) result |= D3D12_COLOR_WRITE_ENABLE_GREEN;
        if (!!(EColorMask::B & type)) result |= D3D12_COLOR_WRITE_ENABLE_BLUE;
        if (!!(EColorMask::A & type)) result |= D3D12_COLOR_WRITE_ENABLE_ALPHA;
    }
    return result;
}
...
GENERATE_CONVERSION_FUNCTION(GetDX12ResourceLayout,
    ...
    CONVERSION_TYPE_ELEMENT(EResourceLayout::SHADER_READ_ONLY, D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE | D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE),
    ...
    CONVERSION_TYPE_ELEMENT(EResourceLayout::DEPTH_READ_ONLY, D3D12_RESOURCE_STATE_DEPTH_READ | D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE | D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE),
    CONVERSION_TYPE_ELEMENT(EResourceLayout::STENCIL_ATTACHMENT, D3D12_RESOURCE_STATE_DEPTH_WRITE),
    CONVERSION_TYPE_ELEMENT(EResourceLayout::STENCIL_READ_ONLY, D3D12_RESOURCE_STATE_DEPTH_READ | D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE | D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE),
...
struct jCreatedResource : public std::enable_shared_from_this<jCreatedResource>
{
    ...
    // Layout management - stored and saved to Pool on Free()
    void SetLayout(EResourceLayout InNewLayout) { Layout = InNewLayout; }
