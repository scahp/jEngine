    ...
    {
        {
            ...
            for(int32 i=0;i<(int32)RenderPassInfo.Subpasses.size();++i)
            {
                ...
                {
                    const VkAttachmentReference attchmentRef = { attachmentIndex, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL };
                    InputAttachmentRefs.emplace_back(attchmentRef);
                }
                for (const int32& attachmentIndex : subPass.OutputColorAttachments)
                {
                    const VkAttachmentReference attchmentRef = { attachmentIndex, VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL };
                    OutputColorAttachmentRefs.emplace_back(attchmentRef);
                }
                if (subPass.OutputDepthAttachment)
                {
                    const VkAttachmentReference attchmentRef = { subPass.OutputDepthAttachment.value(), VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL };
                ...
                SubpassDependencies[DependencyIndex].dstStageMask = VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT;                // todo 최적화를 위해서 지정가능해야 함
        ...
        //    //RenderPassBeginInfo.dependencyCount = (int32)dependencies.size();
        //    //RenderPassBeginInfo.pDependencies = dependencies.data();
