...
public:
    ...
    ~ImGUI()
    {
        ...
        vkDestroySampler(g_rhi_vk->device, sampler, nullptr);
        vkDestroyPipelineCache(g_rhi_vk->device, pipelineCache, nullptr);
        //vkDestroyPipeline(g_rhi_vk->device, pipeline, nullptr);
    ...
    VkPipeline CreatePipelineState(VkRenderPass renderPass, VkQueue copyQueue)
    {
        ...
        imageInfo.initialLayout = VK_IMAGE_LAYOUT_UNDEFINED;
        check(VK_SUCCESS == vkCreateImage(g_rhi_vk->device, &imageInfo, nullptr, &fontImage));
        VkMemoryRequirements memReqs;
        vkGetImageMemoryRequirements(g_rhi_vk->device, fontImage, &memReqs);
        VkMemoryAllocateInfo memAllocInfo{};
        memAllocInfo.sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO;
        memAllocInfo.allocationSize = memReqs.size;
        memAllocInfo.memoryTypeIndex = g_rhi_vk->FindMemoryType(memReqs.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
        check(VK_SUCCESS == vkAllocateMemory(g_rhi_vk->device, &memAllocInfo, nullptr, &fontMemory));
        check(VK_SUCCESS == vkBindImageMemory(g_rhi_vk->device, fontImage, fontMemory, 0));
        ...
        viewInfo.subresourceRange.layerCount = 1;
        check(VK_SUCCESS == vkCreateImageView(g_rhi_vk->device, &viewInfo, nullptr, &fontView));
        ...
        check(g_rhi_vk->CreateBuffer(VK_BUFFER_USAGE_TRANSFER_SRC_BIT
            , VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, uploadSize, stagingBuffer, stagingBufferMemory));
        ...
        VkCommandBuffer copyCmd = g_rhi_vk->BeginSingleTimeCommands();
        ...
        g_rhi_vk->EndSingleTimeCommands(copyCmd);
        ...
        check(VK_SUCCESS == vkCreateSampler(g_rhi_vk->device, &samplerInfo, nullptr, &sampler));
        ...
        check(VK_SUCCESS == vkCreateGraphicsPipelines(g_rhi_vk->device, pipelineCache, 1, &pipelineCreateInfo, nullptr, &pipeline));
