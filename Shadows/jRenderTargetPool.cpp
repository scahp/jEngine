...
struct jTexture* jRenderTargetPool::GetNullTexture(ETextureType type)
{
	...
	{
		static auto temp = jRenderTargetPool::GetRenderTarget({ ETextureType::TEXTURE_2D, ETextureFormat::RGBA, ETextureFormat::RGBA, EFormatType::BYTE, EDepthBufferType::DEPTH, 2, 2, 1 });
		return temp->GetTexture();
	}
	case ETextureType::TEXTURE_CUBE:
	{
		static auto temp = jRenderTargetPool::GetRenderTarget({ ETextureType::TEXTURE_CUBE, ETextureFormat::RGBA, ETextureFormat::RGBA, EFormatType::BYTE, EDepthBufferType::DEPTH, 2, 2, 1 });
		return temp->GetTexture();
	}
	default:
		JASSERT(0);
