...
void jForward_ShadowVolume_Pipeline::Do(const jPipelineContext& pipelineContext) const
{
	...
	g_rhi->SetBlendFunc(EBlendFactor::ONE, EBlendFactor::ZERO);
	...
	g_rhi->SetDepthFunc(ECompareOp::LESS);
	...
	for(auto& light : pipelineContext.Lights)
	{
		...
		g_rhi->SetStencilFunc(ECompareOp::ALWAYS, 0, 0xff);
		g_rhi->SetDepthFunc(ECompareOp::LESS);
		...
		g_rhi->SetStencilFunc(ECompareOp::EQUAL, 0, 0xff);
		...
		g_rhi->SetDepthFunc(ECompareOp::EQUAL);
		g_rhi->SetBlendFunc(EBlendFactor::ONE, EBlendFactor::ONE);
	...
	g_rhi->EnableBlend(true);
	g_rhi->SetBlendFunc(EBlendFactor::SRC_ALPHA, EBlendFactor::ONE_MINUS_SRC_ALPHA);
	g_rhi->SetDepthFunc(ECompareOp::LESS);
	g_rhi->SetDepthMask(true);
	g_rhi->SetColorMask(true, true, true, true);
	g_rhi->EnableStencil(false);
